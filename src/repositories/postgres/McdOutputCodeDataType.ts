 ...
 * 出力データタイプテーブルエンティティ
 */
@Entity('mcd_output_code_data_type')
export default class McdOutputCodeDataType {
    /**
     * ID
     */
     ...
     * 出力データ収集アクターID
     */
    @Column({ type: 'bigint', nullable: false, name: 'mcd_output_code_actor_id' })
    mcdOutputCodeDataActorId: number;
    ...
    constructor (entity?: {}) {
        if (entity) {
            const entityName = 'mcd_output_code_data_type_';
            this.id = parseInt(entity[entityName + 'id']);
            this.mcdOutputCodeDataActorId = entity[entityName + 'mcd_output_code_actor_id'];
