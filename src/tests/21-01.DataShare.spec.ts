...
describe('book-mange API', () => {
    ...
    /**
     * データ共有定義追加（個人）API（非推奨）
     */
    describe('データ共有定義追加（個人）API（非推奨）', () => {
    ...
    /**
     * データ共有定義取得（個人）API（非推奨）
     */
    describe('データ共有定義取得（個人）API（非推奨）', () => {
        test('異常：運営', async () => {
            const response = await supertest(expressApp)
                .get(urljoin(Url.sharePersonURI, '?app=1000009'))
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                .set({ session: JSON.stringify(Session.pxrRoot) })
                .send();
        ...
        test('異常：ワークフロー', async () => {
            const response = await supertest(expressApp)
                .get(urljoin(Url.sharePersonURI, '?wf=1000007'))
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                .set({ session: JSON.stringify(Session.pxrRootInd) })
                .send();
    ...
    /**
     * データ共有定義削除（個人）API（非推奨）
     */
    describe('データ共有定義削除（個人）API（非推奨）', () => {
        test('異常：運営メンバー', async () => {
            const response = await supertest(expressApp)
                .delete(urljoin(Url.sharePersonURI, '1'))
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                .set({ session: JSON.stringify(Session.pxrRoot) })
                .send();
        ...
        test('異常：該当共有定義なし', async () => {
            const response = await supertest(expressApp)
                .delete(urljoin(Url.sharePersonURI, '9999'))
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                .set({ session: JSON.stringify(Session.pxrRootInd) })
                .send();
        ...
        test('異常：該当共有定義データ種なし', async () => {
            ...
            `);
            const response = await supertest(expressApp)
                .delete(urljoin(Url.sharePersonURI, '99'))
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                .set({ session: JSON.stringify(Session.pxrRootInd) })
                .send();
