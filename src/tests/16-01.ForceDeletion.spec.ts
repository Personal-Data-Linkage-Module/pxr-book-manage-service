...
import { OperatorService, CatalogService } from './16-00.StubServer';
...
describe('Book Manage Service.Book Deletion API Controller', () => {
    ...
    test('異常系：Bookが存在しない状態でのBook閉鎖', async () => {
        ...
        expect(JSON.stringify(response.body)).toBe(JSON.stringify({
            status: 400,
            message: '指定されたPXR-IDは登録されていません'
        }));
        expect(response.status).toBe(400);
    });
    test('異常系：Bookが存在しない状態でのBook閉鎖の解除', async () => {
        const response = await supertest(expressApp)
            .put('/book-manage/force-enable')
            .set({
                'Content-Type': 'application/json',
                accept: 'application/json'
            })
    ...
    test('異常系：Bookは存在するが、閉鎖状態ではないのに解除', async () => {
        const response = await supertest(expressApp)
            .put('/book-manage/force-enable')
            .set({
                'Content-Type': 'application/json',
        ...
        expect(JSON.stringify(response.body)).toBe(JSON.stringify({
            status: 400,
            message: '対象は既に閉鎖されている、もしくは閉鎖を解除されています'
        }));
        expect(response.status).toBe(400);
    });
    test('正常系：Bookの強制閉鎖', async () => {
        const response = await supertest(expressApp)
            .put('/book-manage/force-deletion')
            .set({
                'Content-Type': 'application/json',
    ...
    test('異常系：Bookは存在するが、閉鎖状態なのに閉鎖', async () => {
        const response = await supertest(expressApp)
            .put('/book-manage/force-deletion')
            .set({
                'Content-Type': 'application/json',
