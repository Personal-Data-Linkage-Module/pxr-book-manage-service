...
import { CatalogService, OperatorServiceNotFound } from './16-00.StubServer';
...
const operatorService = new OperatorServiceNotFound();
...
describe('Book Manage Service.Book Deletion API Controller', () => {
    ...
    test('異常系：オペレーターサービスにPXRユーザーが存在しない', async () => {
        const response = await supertest(expressApp)
        ...
        expect(JSON.stringify(response.body)).toBe(JSON.stringify({ status: 400, message: 'PXR-IDのユーザーは存在しません' }));
