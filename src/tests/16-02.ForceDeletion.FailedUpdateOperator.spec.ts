...
import { CatalogService, OperatorServiceFailedUpdate } from './16-00.StubServer';
...
const operatorService = new OperatorServiceFailedUpdate();
...
describe('Book Manage Service.Book Deletion API Controller', () => {
    ...
    test('異常系：オペレーターの更新に失敗する', async () => {
        ...
        expect(JSON.stringify(response.body)).toBe(JSON.stringify({ status: 500, message: 'オペレーターサービスへの更新リクエストに失敗しました' }));
