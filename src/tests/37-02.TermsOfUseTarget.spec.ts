...
describe('book-mange API', () => {
     ...
     * Region利用規約再同意通知設定
     */
    describe('Region利用規約再同意通知設定', () => {
        test('正常：Cookie使用、運営', async () => {
            // スタブサーバー起動
            _operatorServer = new StubOperatorServerType0(200, 3);

            // 対象APIに送信
            const response = await supertest(expressApp).post(Url.termsOfUseRegion + '/target')
     ...
     * PF利用規約再同意通知設定
     */
    describe('PF利用規約再同意通知設定', () => {
        test('正常：Cookie使用、運営', async () => {
            // スタブサーバー起動
            _operatorServer = new StubOperatorServerType0(200, 3);

            // 対象APIに送信
            const response = await supertest(expressApp).post(Url.termsOfUsePlatform + '/target')
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                .set('Cookie', ['operator_type3_session=' + 'd89171efae04aa55357bdd2ebf8338725c8fd17ffdfbe61be66ca96c7590b296'])
                .send({});
