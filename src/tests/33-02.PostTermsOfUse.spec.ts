    ...
    describe('Region利用規約同意API', () => {
        ...
        describe('正常系', () => {
            test('更新対象なし', async () => {
                ...
                try {
                    // 対象APIに送信
                    const response = await supertest(expressApp).post(Url.termsOfUseRegion)
    ...
    describe('Platform利用規約同意API', () => {
        ...
        describe('正常系', () => {
            test('指定バージョン以前の利用規約更新通知がある', async () => {
                ...
                try {
                    // 対象APIに送信
                    const response = await supertest(expressApp).post(Url.termsOfUsePlatform)
                        .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                        .set('Cookie', ['operator_type0_session=ee59bbc7fa4d13d853f65c412fe57a3c79af64ee710f3719b5485039d227bd8a'])
                        .send(JSON.stringify({
                            _code: {
